...
return StreamBuilder<DocumentSnapshot>(
  ...
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      ...
      if (roles.contains('Dosen')) {
        ...
        return buildDashboardStaffUI(context, 'Dosen'); // Sesuaikan dengan UI Anda
      } else if (roles.contains('Kaprodi')) {
        print('User is a Kaprodi');
        return buildDashboardStaffUI(context, 'Kaprodi'); // Sesuaikan dengan UI Anda
      } else {
  ...
  Widget buildDashboardStaffUI(BuildContext context, String role) {
...
class DashboardStaffContainer extends StatelessWidget {
  final double width;
  final Widget child;

  const DashboardStaffContainer({
    required this.width,
    required this.child,
    super.key,
  });
...
class DashboardStaffButton extends StatelessWidget {
  final IconData icon;
  final Text content;

  const DashboardStaffButton({
    required this.icon,
...
class _AssignRuangKelasButton extends RouteButton {
  const _AssignRuangKelasButton()
      : super(
          icon: Icons.app_registration_rounded,
          iconColor: Colors.black,
          content: "Ruang Kelas",
